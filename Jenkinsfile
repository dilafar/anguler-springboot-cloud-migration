    ...
    stages{
                ...
                dir('employeemanagerfrontend') {
                    script {
                            withCredentials([usernamePassword(credentialsId: 'docker-hub', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
                                ...
                                sh 'docker build -t fadhiljr/nginxapp:employee-frontend-v41 .'
                                sh "echo $PASS | docker login -u $USER --password-stdin"
                                sh 'docker push fadhiljr/nginxapp:employee-frontend-v41'
                                sh 'cosign version'
        ...
        stage("change image in kubeconfig") {
            steps {
                dir('kustomization') {
                    script {
                        sh "sed -i 's#replace#fadhiljr/nginxapp:employee-frontend-v41#g' frontend-deployment.yml" 
