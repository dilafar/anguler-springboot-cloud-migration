    ...
    stages{
                ...
                dir('employeemanagerfrontend') {
                    script {
                            withCredentials([usernamePassword(credentialsId: 'docker-hub', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
                                ...
                                sh 'docker build -t fadhiljr/nginxapp:employee-frontend-v45 .'
                                sh "echo $PASS | docker login -u $USER --password-stdin"
                                sh 'docker push fadhiljr/nginxapp:employee-frontend-v45'
                                sh 'cosign version'

                                sh '''
                                    IMAGE_DIGEST=$(docker inspect --format='{{index .RepoDigests 0}}' fadhiljr/nginxapp:employee-frontend-v45)
        ...
        stage("change image in kubeconfig") {
            steps {
                dir('kustomization') {
                    script {
                        sh "sed -i 's#replace#fadhiljr/nginxapp:employee-frontend-v45#g' frontend-deployment.yml" 
